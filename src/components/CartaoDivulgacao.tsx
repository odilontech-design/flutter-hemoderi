...
/**
 * O material de divulgação da clínica: link curto e QR Code, prontos para
 * imprimir ou mandar no WhatsApp.
 */
export async function CartaoDivulgacao({ slug, nome }: { slug: string; nome: string }) {
  const url = urlDeDivulgacao(slug);
  const qr = await qrCodeDataUri(url);
